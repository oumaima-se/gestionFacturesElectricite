...
//include("protectionAdmin.php");
    ...
    // connexion à la bdd:
    $mysqli=mysqli_connect('localhost','root','','tp2_web','3306');//'serveur','nom d'utilisateur','pass','nom de la table'
    if(!$mysqli) {
    ...
    $UrlPage = "reclamation.php"; // cette variable vous permet de modifier les lien de cette page très facilement au cas où vous utiliseriez le script sur une autre page
    $Retour = '<br/><p><a href="'.$UrlPage.'" class ="btn ajouter">Retour aux tickets</a></p>'; // quand une action est demandée, on affiche ce lien pour que le membre puisse revenir en arrière
    
    //--------------
    //FIN DE SECTION
    //--------------
$_SESSION[$NomDeSessionPourId]=$id_client;
...
<center>
                    ...
                    if($AfficherFormulaire == 1) {
                        ...
                        <form action="<?php echo $UrlPage; ?>?action=repondre&amp;id=<?php echo $IdTicket; ?>" method="post">
                            <table>
                                ...
                                <tr>
                                    <td></td>
                                    <td><input type="submit" class ='btn afficher' name="repondre" value="Répondre" /></td>
                                </tr>
                            </table>
                        </form>
                        <?php
                ...
                if(mysqli_num_rows($req) != 1) {
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <td>
                                <center>
                                    <a href="<?php echo $UrlPage; ?>?action=repondre&amp;id=<?php echo $IdTicket; ?>" class='btn afficher'>Ajouter une réponse</a>  
                                </center>
                                <br />
                                <?php
            ...
            if(!isset($_GET['id']) OR !preg_match("#^[0-9]+$#",$_GET['id'])) { // on demande si l'id est dans l'url et si l'id est un chiffre
                ...
                if(mysqli_num_rows($req) != 1) {
                    ...
                    if(!isset($_GET['supprimer'])){//vérification de certitude...
                        ?>
                        <p>&Ecirc;tes vous sûr ? <a href="<?php echo $UrlPage; ?>?action=supprimer&amp;id=<?php echo $IdTicket; ?>&amp;supprimer" class ='btn ajouter'>Oui</a></p>
                        <?php
                    } else {
            ...
            if(!isset($_GET['id']) OR !preg_match("#^[0-9]+$#",$_GET['id'])) { // on demande si l'id est dans l'url et si l'id est un chiffre
                ...
                if(mysqli_num_rows($req) != 1) {
                    ...
                    if(!isset($_GET['fermer'])){//vérification de certitude...
                        ?>
                        <p>&Ecirc;tes vous sûr ? <a href="<?php echo $UrlPage; ?>?action=fermer&amp;id=<?php echo $IdTicket; ?>&amp;fermer" class ='btn ajouter'>Oui</a></p>
                        <?php
                    } else {
                        //on ferme le ticket
    ...
    if($AfficherLesTickets == 1) {
        if($IdMembre==$IdAdmin){//on affiche les tickets ouverts pour l'admin:
            ...
            $req = mysqli_query($mysqli,"SELECT * FROM tickets");
            if(mysqli_num_rows($req) == 0) {
                ...
                while($info = mysqli_fetch_assoc($req)) {
                    ...
                    <tr>
                        ...
                        <td><?php echo $info['sujet']; ?></td>
                        <td><?php echo $ArrayPriorites[$info['priorite']]; ?></td>
                        <td><?php echo $info['etat'] == 1 ? "<span style='color:blue;font-weight:bold'>Ouvert</span>" : "<span style='color:gray;font-weight:bold'>Fermé</span>"; ?></td>
                        <td>
                            <a href="<?php echo $UrlPage; ?>?action=afficher&amp;id=<?php echo $info['id']; ?>" class='btn afficher'>Afficher ce ticket</a>
                            
                            <a href="<?php echo $UrlPage; ?>?action=fermer&amp;id=<?php echo $info['id']; ?>" class='btn modifier'>Fermer ce ticket</a>
                            
                            <a href="<?php echo $UrlPage; ?>?action=supprimer&amp;id=<?php echo $info['id']; ?>" class='btn supprimer'>Supprimer ce ticket</a>
            ...
            if(mysqli_num_rows($req) == 0) {
                ...
                while($info = mysqli_fetch_assoc($req)) {
                    ...
                    <tr>
                        ...
                        <td><?php echo $info['sujet']; ?></td>
                        <td><?php echo $ArrayPriorites[$info['priorite']]; ?></td>
                        <td><?php echo $info['etat'] == 1 ? "<span style='color:green;font-weight:bold'>Ouvert</span>" : "<span style='color:gray;font-weight:bold'>Fermé</span>"; ?></td>                        <td>
